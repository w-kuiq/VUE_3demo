            ...
            <tr v-for="(item,index) in productList">
                ...
                <td class="td-product"><img src="testimg.jpg" width="98" height="98">
                    <div class="product-info">
                        ...
                        <p>规格/纯度:{{item.pro_depot}}&nbsp;&nbsp;起定量：{{item.pro_min}}</p>
                ...
                <td class="td-do"><a href="javascript:;" class="product-delect" @click="deleteOnePro(index)">删除</a></td>
            </tr>
          </tbody>
        </table>
    </div>
    <div class="cart-product-info">
        <a class="delect-product" href="javascript:;" @click="deleteChoosePro"><span></span>删除所选商品</a>
...
export default {
      ...
      productList: [
        {
          ...
          pro_num: 3, //数量
          pro_img: "../../assets/testimg.jpg", //图片链接
          pro_price: 800.37, //单价
          select: false
        },
        {
          pro_name: "运动鞋", //产品名称
          pro_brand: "adidas", //品牌名称
          pro_place: "美国", //产地
          pro_purity: "41", //规格
          pro_min: "1双", //最小起订量
          pro_depot: "杭州仓海仓储", //所在仓库
          pro_num: 30, //数量
          pro_img: "../../assets/testimg.jpg", //图片链接
          pro_price: 699.99, //单价
          select: false
  ...
  computed: {
    ...
    //计算件数和总价
    getTotal: function() {
      var _productList = this.productList.filter(function(item) {
          return item.select;
        }),
        totalPrice = 0;
      for (let i = 0; i < _productList.length; i++) {
        totalPrice += _productList[i].pro_num * _productList[i].pro_price;
      }
      return {totalNum:_productList.length,totalPrice:totalPrice}
    }
  },
  methods: {
    //全选功能
    selectProduct: function (is_select) {
      for (let i = 0; i < this.productList.length; i++) {
        this.productList[i].select = !is_select;
      }
    },
    //删除已选商品
    deleteChoosePro:function(){
      this.productList = this.productList.filter(function(item){
        return !item.select
      })
    },
    //删除单挑商品
    deleteOnePro:function (index) {
...
.page-shopping-cart .check-span {
  ...
  width: 24px;
  height: 20px;
  background: url("../assets/shop.png") no-repeat 0 0;
}
.page-shopping-cart .check-span.check-true {
  background: url("../assets/shop.png") no-repeat 0 -22px;
}
...
.page-shopping-cart .cart-product table .product-num .num-reduce span {
  background: url("../assets/shop.png") no-repeat -40px -22px;
  display: block;
  width: 6px;
  height: 2px;
...
.page-shopping-cart .cart-product table .product-num .num-add span {
  background: url("../assets/shop.png") no-repeat -60px -22px;
  display: block;
  width: 8px;
  height: 8px;
...
.page-shopping-cart .cart-product-info .delect-product span {
  ...
  width: 13px;
  height: 15px;
  background: url("../assets/shop.png") no-repeat -60px 0;
}
...
.page-shopping-cart .cart-product-info .keep-shopping span {
  ...
  width: 15px;
  height: 15px;
  background: url("../assets/shop.png") no-repeat -40px 0;
}
...
.page-shopping-cart .cart-worder .choose-worder span {
  ...
  width: 22px;
  height: 22px;
  background: url("../assets/shop.png") no-repeat -92px 0;
}
...
.choose-worker-box .box-title a {
  ...
  width: 10px;
  height: 10px;
  background: url("../assets/shop.png") no-repeat -80px 0;
}
.choose-worker-box .box-title a:hover {
  background: url("../assets/shop.png") no-repeat -80px -22px;
}
